...
export function MacroFishingBombPage() {
  ...
  async function handleRun() {
    try {
      ...
      if (!result.success) {
        message.error(result.message || "炸鱼宏执行失败");
        setLogs(result.data?.logs || []);
        return;
      }

      message.success(result.message || "炸鱼宏执行完成");
      setLogs(result.data?.logs || []);
    } catch (error) {
      console.error(error);
      message.error(error instanceof Error ? error.message : "炸鱼宏执行异常");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div style={{ padding: 24 }}>
      <Space direction="vertical" size={16} style={{ width: "100%" }}>
        <div>
          <Title level={3}>炸鱼宏</Title>
          <Paragraph type="secondary">
            燕云十六声炸鱼宏，用于执行本地键鼠自动化流程。
          </Paragraph>
        </div>
